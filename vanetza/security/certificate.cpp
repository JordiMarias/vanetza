...
Certificate::Certificate(std::string certificate){
    &(this->certificate) = 0;
    asn_dec_rval_t rval;
    rval = xer_decode(0, &asn_DEF_EtsiTs103097Certificate, (void **)&(&(this->certificate)),
        certificate.c_str(), certificate.size());
    if(rval.code != RC_OK){
        //Must throw an error
        ASN_STRUCT_FREE(asn_DEF_EtsiTs103097Certificate, &(this->certificate));
    }
}
