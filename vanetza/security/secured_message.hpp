...
class SecuredMessageV3{
    public:
        SecuredMessageV3();
        SecuredMessageV3(ByteBuffer oer_message);
        ~SecuredMessageV3();
        uint8_t protocol_version() const { return 3; }
        ByteBuffer serialize();
        //Getters and setters will be written while developing
        uint64_t * get_generation_time();
        Psid_t get_psid();
        ThreeDLocation_t* get_generation_location(); 



    private:
        EtsiTs103097Data_t message;
...
using SecuredMessage = SecuredMessageV3;
